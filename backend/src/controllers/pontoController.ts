...
// POST /api/debug/pontos-marcar-videoclipe — correção pontual: WISDOM
// (linha 658, "SA5M - WISDOM TOOTH") e CURSED BLESSED (linha 659,
// "Samantha Cooper - CURSED BLESSED") foram lançados hoje com o "Tipo de
// vídeo" deixado como "Video" em vez de "Music Video" no formulário de
// Gestão — por isso marcarVideoclipeNaPontos nunca rodou (só é chamado
// quando tipo === "music video"). Marca manualmente N (checkbox) e O (data)
// pras duas linhas, confirmadas via /api/debug/pontos-buscar. Só aceita
// linha+musicaEsperada exatos (nunca busca por texto), pra nunca arriscar
// escrever na linha errada. Endpoint temporário, remover após o uso.
export async function debugMarcarVideoclipeController(request: Request): Promise<Response> {
  const body = (await request.json().catch(() => ({}))) as {
    linha?: number;
    musicaEsperada?: string;
    dataFormatada?: string;
  };
  const { linha, musicaEsperada, dataFormatada } = body;

  if (!linha || !musicaEsperada || !dataFormatada) {
    return jsonResponse({ ok: false, error: "linha, musicaEsperada e dataFormatada são obrigatórios." }, 400);
  }

  const cells = await googleSheetsService.registrosCharts.readValues(SHEET, `D${linha}:O${linha}`);
  const row = cells?.[0] || [];
  const musicaAtual = normalizeText(row[0]); // D, primeira coluna do range
  if (normalizeComparison(musicaAtual) !== normalizeComparison(musicaEsperada)) {
    return jsonResponse(
      { ok: false, error: `Linha ${linha} não bate: esperado "${musicaEsperada}", achado "${musicaAtual}".` },
      409,
    );
  }
  const checkboxAtual = normalizeText(row[10]); // N (D=0 ... N=10 dentro do range D:O)
  if (checkboxAtual.toUpperCase() === "TRUE") {
    return jsonResponse({ ok: false, error: `Linha ${linha} já estava marcada — nada feito.` }, 409);
  }

  await googleSheetsService.registrosCharts.updateValues(SHEET, `N${linha}:O${linha}`, [
    ["TRUE", dataFormatada],
  ]);

  return jsonResponse({ ok: true, linha, musica: musicaAtual });
}
