...
import {
  ...
  mesclarAlbunsDuplicadosController,
...
export async function handleEmpireApiRoutes(request: Request): Promise<Response | null> {
  ...
  if (isEmpirePlayForumPath) {
    ...
    } else if (url.pathname === "/api/playlists/albuns/admin/mesclar-albuns") {
      response = await mesclarAlbunsDuplicadosController(request);
    } else if (url.pathname === "/api/playlists/albuns/admin/diagnostico-album") {
      response = await diagnosticoAlbumLegadoController(request);
    } else if (url.pathname === "/api/playlists/albuns/admin/dump-linhas") {
  ...
  } else if (url.pathname === "/api/artistas/biografia") {
    if (request.method !== "POST") {
      return new Response(
        JSON.stringify({ success: false, error: "Use POST para /api/artistas/biografia." }),
        { status: 405, headers: { ...CORS_HEADERS, "Content-Type": "application/json" } },
      );
    }
  ...
  } else if (url.pathname === "/api/forum/atividade-recente") {
    if (request.method !== "GET") {
      return new Response(
        JSON.stringify({ success: false, error: "Use GET para /api/forum/atividade-recente." }),
        { status: 405, headers: { ...CORS_HEADERS, "Content-Type": "application/json" } },
      );
    }
    response = await getAtividadeRecenteController();
  ...
  } else if (url.pathname === "/api/awards/artista") {
    response = await getArtistAwardsController(request);
  } else if (url.pathname === "/api/awards/todos") {
